 ...
 * @return {string} str
 */
function openIframe(pathurl) {
    $('#showH5').show();
    $('.maskIframe').css('display', 'block');
    $('#iframearea').empty().append($('<iframe class="iframeitem" frameborder="0" scrolling="no" marginheight="0" />').attr('src', pathurl));
    // debugger
...
$('#closeBtn').click(function () {
    $('#showH5').hide();
    // $('#iframeLoading').show()
    $('.maskIframe').css('display', 'none');
    $('#iframearea').empty()
})
$('.maskIframe').click(function () {
    $('#showH5').hide();
    // $('#iframeLoading').show()
    $('.maskIframe').css('display', 'none');
    $('#iframearea').empty()
...
function getMessage(msg) {
    ...
    switch (msg.type) {
        ...
        case 'geo':
            ...
            //         url = ossURL + 'appweb' + "/endClassInformation.html?" + token + "&" + content.messageId + "&SELL"
            //         str = '<a class="titles" style="font-size:14px;color:#666">' + content.title + '</a>' + '<a style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></a>';
            //     } else {
            //         str = '<p onclick="windowOpen(' + content + ')" class="titles" style="font-size:14px;color:#666">' + content.title + '</p>' + '<p style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></p>';
            //     }
            // } else
        case 'custom':
            var content = JSON.parse(msg.content);
            var pathurl;
            if (content.type === 301) { // 确认函 确认函ID 订单详情
                if (content.content.courseId) {
                    pathurl = redirectURL + 'modules/vue/seller_detail.html?id=' + content.content.courseId
                    str = '<div><a href=' + pathurl + '  target="_blank" class="titles" style="font-size:14px;color:#666">' + content.title + '</a>' + '<a style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></a></div>';

                } else {
                    str = '<p class="titles" style="font-size:14px;color:#666">' + content.title + '</p>' + '<p style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></p>';
                }
            } else if (content.type === 302) { // 账单 账单ID 我的账单
                if (content.content.courseId) {
                    pathurl = redirectURL + 'modules/vue/bad_order.html'
                    str = '<div><a href=' + pathurl + '  target="_blank" class="titles" style="font-size:14px;color:#666">' + content.title + '</a>' + '<a style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></a></div>';
                } else {
                    str = '<p onclick="windowOpen(' + content + ')" class="titles" style="font-size:14px;color:#666">' + content.title + '</p>' + '<p style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></p>';
                }
            } else if (content.type === 303) { // 确认课堂 课堂ID H5
                if (content.content.courseId) {
                    pathurl = ossURL + 'appweb' + "/tz_arrangingLesson.html?token=" + token + "&id=" + content.messageId + "&status=SELL"
                    str = '<div onclick=\'openIframe("' + pathurl + '")\'><a class="titles" style="font-size:14px;color:#666">' + content.title + '</a>' + '<a style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></a></div>';
                } else {
                    str = '<p onclick="windowOpen(' + content + ')" class="titles" style="font-size:14px;color:#666">' + content.title + '</p>' + '<p style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></p>';
                }
            } else if (content.type == 305) { // 取消排课 课堂ID H5
                if (content.content.courseId) {
                    pathurl = ossURL + 'appweb' + "/tz_cancelLesson.html?token=" + token + "&id=" + content.messageId + "&status=SELL"
                    str = '<div onclick=\'openIframe("' + pathurl + '")\'><a class="titles" style="font-size:14px;color:#666">' + content.title + '</a>' + '<a style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></a></div>';
                } else {
                    str = '<p onclick="windowOpen(' + content + ')" class="titles" style="font-size:14px;color:#666">' + content.title + '</p>' + '<p style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></p>';
                }
            } else if (content.type == 307) { // 开始上课 课堂ID H5
                if (content.content.courseId) {
                    pathurl = ossURL + 'appweb' + "/tz_startLesson.html?token=" + token + "&id=" + content.messageId + "&status=SELL"
                    str = '<div onclick=\'openIframe("' + pathurl + '")\'><a class="titles" style="font-size:14px;color:#666">' + content.title + '</a>' + '<a style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></a></div>';
                }
            } else if (content.type == 308) { // 评价课程 课程ID H5
                if (content.content.courseId) {
                    pathurl = ossURL + 'appweb' + "/endClassInformation.html?token=" + token + "&id=" + content.messageId + "&status=SELL"
                    str = '<div onclick=\'openIframe("' + pathurl + '")\'><a class="titles" style="font-size:14px;color:#666">' + content.title + '</a>' + '<a style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></a></div>';
                } else {
                    str = '<p onclick="windowOpen(' + content + ')" class="titles" style="font-size:14px;color:#666">' + content.title + '</p>' + '<p style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></p>';
                }
            } else if (content.type == 312) { // 分享订单 订单详情
                pathurl = redirectURL + 'modules/vue/seller_detail.html?id=' + content.content.courseId
                //  onclick=\'openIframe("' + pathurl + '")\'
                str = '<div><a href=' + pathurl + '  target="_blank" class="titles" style="font-size:14px;color:#666">' + content.title + '</a>' + '<a style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></a></div>';
            } else if (content.type == 313) { // 分享订单 订单详情
                if (content.content.courseId) {
                    pathurl = ossURL + 'appweb' + "/tz_cancelLesson.html?token=" + token + "&id=" + content.messageId + "&status=SELL"
                    str = '<div><a href=' + pathurl + '  target="_blank" class="titles" style="font-size:14px;color:#666">' + content.title + '</a>' + '<a style="font-size:12px;color:#666"><span>' + content.subtitle + '</span><img src=' + content.imgUrl + '></a></div>';
                } else {
