...
var SDKBridge = function (ctr, data) {
    var crmtoken = readCookie('crmtoken'),
        userUID = readCookie('crmid'),
        that = this;
    if (!crmtoken) {
    ...
    function onTeams(teams) {
        ...
        console.log(teams);
        var a = $(".j-advanceTeam li").attr("data-account");
    }
    ...
    function onMsg(msg) {
        ...
        // var currentImg=0;
        // var imgArr=['chat1.png','chat2.png','chat1.png'];
        //   function changeImg(){
        //     if(currentImg >= 2){currentImg=0} 
        //     else{currentImg++}
        //     var img = $(window.parent.document).find("#newchat");
        //     var img1 = $(window.parent.document).find("#newchat1");
        //     img.css({"background":"url(../views/im/"+imgArr[currentImg]+")","background-size":"100%"});
        //   }
            ...
            var popNotice = function () {
                if (Notification.permission == "granted") {
                    if(msg.content){
                        ...
                        var notification = new Notification("ClassBro在线--通知", {
                            body: contentInfo.subtitle + '\n' + contentInfo.title,
                            icon: 'images/classbroicon.png'
                        });
                    } else {
                        // var notification = new Notification("ClassBro在线--通知", {
                        //     body: msg.text,
                    ...
                    notification.onclick = function () {
                        // text.innerHTML = '用户于' + new Date().toTimeString().split(' ')[0] + '点击了该条消息';
                        notification.close();
                    };
                }
            };
...
SDKBridge.prototype.sendCustomMessage = function (scene, to, content, callback) {
    ...
    alert('正在发送p2p自定义消息, id=' + msg.idClient);
    pushMsg(msg);
};
 ...
 * 获取本地历史记录消息
 */
SDKBridge.prototype.getLocalMsgs = function (sessionId, end, done, keyword) {
    if (end) {
