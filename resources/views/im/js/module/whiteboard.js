...
window.yunXin.WB = new window.Vue({
    ...
    methods: {
        ...
        call: function () {
            ...
            if (
                ...
                window.minAlert.alert({
                    type: 'error',
                    msg: this.isCalled || this.isCalling ?
                        '正在白板互动中，无法发起新的白板互动' :
                        '正在音视频通话中，无法发起白板互动',
                    cancelBtnMsg: '知道了'
                });
                ...
                cbConfirm: function (data) {
                    ...
                    if (data.type === 'webnet') {
                        ...
                        if (!window.yunXin.myNetcall.checkRtcBrowser()) {
                            return window.minAlert.alert({
                                type: 'error',
                                msg: '当前浏览器不支持WebRTC，无法使用白板互动功能。（白板功能中的音频功能需要WebRTC的支持）请使用最新版Chrome浏览器',
                                cancelBtnMsg: that.isCalled ? '拒绝邀请' : '结束邀请',
                                cbCancel: function () {
                                    data.incompatible = true;
                                    failCallback && failCallback(data);
        ...
        // 接收白板sdk的数据
        onData: function (obj) {
            this.drawPlugin && this.drawPlugin.act({
                account: obj.account,
                data: obj.data
            })
        }
    ...
    mounted: function () {
        ...
        this.initDrawPlugin()
        this.$refs.container.ondragstart = function () {
            return false
        };
        // Vue已经把方法绑定好了this
        // 点击互动白板按钮，发起邀请
