...
YX.fn.cbInitInfo = function (error, data) {
    if (!error) {
        ...
        // 存储 要搜索的数组
        // console.error(data);
        // console.error(this.cache.getTeamlist());
    } else {
        alert('获取用户信息失败');
    }
};
...
YX.fn.openChatBox = function (account, scene) {
    ...
    //根据帐号跟消息类型获取消息数据
    if (scene == 'p2p') {
        this.$orderInfo && this.$orderInfo.addClass('hide');
        ...
        this.$orderInfo && this.$orderInfo.removeClass('hide');
...
YX.fn.switchPanel = function (evt) {
    ...
    if (type === 'sessions') {
        ...
        var crm_number = localStorage.getItem("crm_number");
        $.ajax({
            type: "get",
            dataType: "json",
            data: {
            ...
            url: "http://www.classbro.ca/api/student/caller/list",
            success: function (json) {
                console.error(json);
                if (json.status == 200) {
                    $("#touristul").empty("");
                    json.body.list.map(function (item, index) {
                        if (item.statused == 4) {
                            item.statused = "结束咨询"
                            item.style = "red"
                        } else {
                        ...
                        var html = "<li class='panel_item'><span class='span'>" + item.callerImAccount + "</span><span class='accept " + item.style + "' data-account=" + item.id + ">" + item.statused + "</span></li>";
                        $("#touristul").append(html);
                    });
                    var id = $(".accept").attr("data-account");
                        ...
                        if ($(this).html() == "接受访客") {
                            $.ajax({
                                ...
                                data: {
                                    "token": token,
                                    "id": id,
                                    "customerId": crm_number
                                },
                                url: "http://www.classbro.ca/api/seller/caller/acceptCaller",
                                success: function (json) {
                                    console.error(json);
                                    if (json.status == 200) {
                                        $(this).html("结束咨询")
                                        $("#rightPanel").css("display", "block")
                                        $("#switchPanel a:first").click();
                                        $("#sessions ul li:first").click();
                                    } else {
                                        alert(json.body.msg)
                                        $("#switchPanel a:first").click();
                                        $("#sessions ul li:first").click();
                            ...
                            $.ajax({
                                ...
                                url: "http://www.classbro.ca/api/seller/caller/finishedCaller",
                                success: function (json) {
                                    console.error(json);
                                    if (json.status == 200) {
                                        //   nim.deleteSessions({
                                        ...
                                        //         to: "visi206"
                                        //     }],
                                        //     done: function deleteSessionsDone(error, obj) {
                                        //       console.log(error);
                                        //       console.log(obj);
...
YX.fn.doPoint = function () {
    ...
    } else if (!this.$touristWrap.is('.hide')) {
        $container = this.$touristWrap;
    } else {
