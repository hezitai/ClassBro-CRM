...
YX.fn.message = function () {
    ...
    this.$maskerUploadFiles = $('#masker-upload-files');

    /* 
     *超级搜索 
     * */
    this.$superSearchBtn = $('#superSearch');
    this.$superSearchVal = $('#superSearchVal');
    this.$superSearchBtn.on('click', this.superSearchGetLocalMsgs.bind(this));
 ...
 * @return
 */
YX.fn.superSearchGetLocalMsgs = function () {
    console.log(this.$superSearchVal.val());
    var keyword = this.$superSearchVal.val();
    var _this = this;
    if (keyword == '') {
        alert('请输入要搜索的内容')
    } else {
        ...
        //     limit: 80,
        //     done: this.superSearchGetLocalMsgsDone.bind(this),
        //     keyword: '',
        // });
        // var session = this.cache.getSessions();
        var teamlist = this.cache.getTeamlist();
        var userList = this.cache.getFriendslistOnShow();

        // console.log(session);
        console.log(teamlist);
        console.log(userList);
        var keyword = this.$superSearchVal.val().replace(/ /g,'');
        var readySearch = [];
        var searchUserListResult = [];
        // var searchSessionListResult = [];
        var searchTeamListResult = [];
        // readySearch = session;
        // for (var i = 0; i < readySearch.length; i++) {
        //     var n = readySearch[i].lastMsg.fromNick.indexOf(keyword);
        //     if (n > -1) {
        //         searchSessionListResult.push(readySearch[i]);
        //     }
        // };
        for (var i = 0; i < teamlist.length; i++) {
            var n = teamlist[i].name.indexOf(keyword);
            if (n > -1) {
                searchTeamListResult.push(teamlist[i]);
            }
        }
        for (var i = 0; i < userList.length; i++) {
            var n = userList[i].nick.indexOf(keyword);
            if (n > -1) {
                searchUserListResult.push(userList[i]);
            }
        }
        // console.log(searchSessionListResult);
        console.log(searchTeamListResult); //群组
        console.log(searchUserListResult); //好友
        $('#superSearchResultUl').empty();
        $('#superSearchResultBox').removeClass("hide");
        $('#resultMask').removeClass('hide');
        if (searchTeamListResult.length != 0) {
            var searchTeamListResultEl = $('<li><p style="padding:5px;">群组：</p</li>');
            $('#superSearchResultUl').append(searchTeamListResultEl);
            for (var i = 0; i < searchTeamListResult.length; i++) {
                // <p>' + searchTeamListResult[i].name + ':</p>
                var html = $('<li class="superSearchResultLi" style="padding:10px;" title="' + searchTeamListResult[i].name + '">' + searchTeamListResult[i].name + '</li>');
                $('#superSearchResultUl').append(html);
                (function (i) {
                    html.bind('click', function () {
                        console.log(searchTeamListResult[i]);
                        // var type = searchTeamListResult[i].scene;
                        var type = 'team';
                        var id = searchTeamListResult[i].teamId;
                        // if (type == 'p2p') {
                        //     id = searchTeamListResult[i].from;
                        // } else if (type == 'team') {
                        //     id = searchTeamListResult[i].to;
                        // }
                        _this.openChatBox(id, type);
                        $('#resultMask').addClass("hide");
                        $('#superSearchResultBox').addClass("hide");
                    });
                })(i)
            };
        } else {
            var searchTeamListResultEl = $('<li><p style="padding:5px;">群组：</p</li>');
            var searchTeamListResultElnone = $('<li><p style="padding:10px;">暂无搜索结果</p</li>');
            $('#superSearchResultUl').append(searchTeamListResultEl).append(searchTeamListResultElnone);
        }
        if (searchUserListResult.length != 0) {
            var searchUserListResultEl = $('<li><p style="padding:5px;">好友：</p</li>');
            $('#superSearchResultUl').append(searchUserListResultEl);
            for (var i = 0; i < searchUserListResult.length; i++) {
                // <p>' + searchUserListResult[i].name + ':</p>
                var html = $('<li class="superSearchResultLi" style="padding:10px;" title="' + searchUserListResult[i].nick + '">' + searchUserListResult[i].nick + '</li>');
                $('#superSearchResultUl').append(html);
                (function (i) {
                    html.bind('click', function () {
                        console.log(searchUserListResult[i]);
                        // var type = searchUserListResult[i].scene;
                        var type = 'p2p';
                        var id = searchUserListResult[i].account;
                        // if (type == 'p2p') {
                        //     id = searchTeamListResult[i].from;
                        // } else if (type == 'team') {
                        //     id = searchTeamListResult[i].to;
            ...
            var searchUserListResultEl = $('<li><p style="padding:5px;">好友：</p</li>');
            var searchUserListResultElnone = $('<li><p style="padding:10px;">暂无搜索结果</p</li>');
            $('#superSearchResultUl').append(searchUserListResultEl).append(searchUserListResultElnone);
        }
        $('#resultMask').on('click', function () {
            $('#resultMask').addClass("hide");
            $('#superSearchResultBox').addClass("hide");
        })
    }
...
// YX.fn.onSessions = function (sessions) {
//     console.log('收到会话列表', sessions);
//     data.sessions = nim.mergeSessions(data.sessions, sessions);
//     updateSessionsUI();
// }
// YX.fn.onUpdateSession = function (session) {
//     console.log('会话更新了', session);
//     data.sessions = nim.mergeSessions(data.sessions, session);
//     updateSessionsUI();
// }
// YX.fn.updateSessionsUI = function () {
//     // 刷新界面
// }
YX.fn.superSearchGetLocalMsgsDone = function (err, data) {
    var _this = this;
    if (!err) {
        console.log(data);
        ...
        if (array.length == 0) {
            var html = $('<p style="padding:5px;">暂无搜索结果</p>');
            $('#superSearchResultUl').append(html);
        } else {
...
YX.fn.superSearchGetTeam = function (err, teams) {
    if (!err) {
        console.log(teams);
    } else {
        alert('搜索失败，请刷新重试');
...
YX.fn.uploadFile = function () {
    this.$maskerUploadFiles.removeClass('hide');
    var that = this,
...
YX.fn.sendTextMessage = function (data) {
    var sss = "crm" + localStorage.getItem("crm_number");
    var scene = this.crtSessionType,
        to = this.crtSessionAccount,
...
YX.fn.sendMsgDone = function (error, msg) {
    ...
    console.log(JSON.parse(msg.pushPayload));
    this.cache.addMsgs(msg);
    ...
    this.$maskerUploadFiles.addClass('hide');
};
