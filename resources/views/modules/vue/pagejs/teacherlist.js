var vm = new Vue({
    ...
    methods: {
        ...
        getTableData: function () {
            ...
            $.ajax({
                ...
                success: function (json) {
                    ...
                    //badComment  goodComment
                    for (var j in self.tableDataEnd) {
                        self.tableDataEnd[j].goodComment = 0;
                        self.tableDataEnd[j].badComment = 0;
                        for (var i in self.tableDataEnd[j].teacSmallSnitchsExt) {
                            if (self.tableDataEnd[j].teacSmallSnitchsExt[i].snType == 0) {
                                self.tableDataEnd[j].goodComment++;
                            }
                            if (self.tableDataEnd[j].teacSmallSnitchsExt[i].snType == 1) {
                                self.tableDataEnd[j].badComment++;
                            }
                        }
                    }

                    // console.error(self.tableDataEnd);
        ...
        view_detail(row, index) {
            localStorage.setItem('teacherlist', JSON.stringify(row));
            window.open('teacherlist-updata.html');
            // if (row.type == 1) {
            //     vm.istrue = false
