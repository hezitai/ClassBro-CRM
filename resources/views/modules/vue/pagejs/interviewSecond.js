    ...
    $('#calendar').fullCalendar({
        ...
        eventClick: function (calEvent, jsEvent, view) { //日程区块，单击时触发
            ...
            if (calEvent.statused == 2) {
                vm.resultmodal = true;
            } else if (calEvent.statused != 2) {
                vm.getOperList();
                vm.interviewed = true;
                vm.roomUrl = vm.clickInfo.reexamineRoomNum;
                vm.roomID = vm.clickInfo.reexamineJoinUrl;
                vm.getOperIds = vm.clickInfo.reexamineInterviewer;
                vm.teacId = vm.clickInfo.teacId;
                vm.newStartTime = vm.clickInfo.reexamineTime;
                vm.newEndTime = vm.clickInfo.reexamineEndTime;
            }
            // if (calEvent.statused < 2) {
        ...
        eventDrop: function (event, delta, revertFunc, jsEvent, ui, view) { //日程拖拽停止并且已经拖拽到其它位置了
            ...
            if (delta._milliseconds >= 0 || delta._days >= 0 || delta._months >= 0) {
                vm.newStartTime = new Date(event.reexamineTime).getTime() + years +
                    months + days + hours + minutes + seconds + milliseconds;
                vm.newEndTime = new Date(event.reexamineEndTime).getTime() + years +
                    months + days + hours + minutes + seconds + milliseconds;
            } else {
                vm.newStartTime = new Date(event.reexamineTime).getTime() - years -
                    months - days - hours - minutes - seconds - milliseconds;
            ...
            // new Date(event.reexamineTime).getTime()
            ...
            var thisType = view.type;
            vm.teacId = event.teacId;
            vm.getOperIds = event.reexamineInterviewer;
            vm.roomUrl = event.reexamineRoomNum;
            vm.roomID = event.reexamineJoinUrl;
            // debugger;
            vm.chooseOperSubmit();
            // agendaDay
            // agendaWeek
        ...
        eventResize: function (event, delta, revertFunc, jsEvent, ui, view) { //日程大小调整完成并已经执行更新时触发
            ...
            milliseconds = delta._data.milliseconds;
            if (delta._milliseconds >= 0 || delta._days >= 0 || delta._months >= 0) {
                vm.newStartTime = new Date(event.reexamineTime).getTime()
                vm.newEndTime = new Date(event.reexamineEndTime).getTime() + years +
                    months + days + hours + minutes + seconds + milliseconds;
            } else {
                vm.newStartTime = new Date(event.reexamineTime).getTime()
        ...
        events: function (start, end, timezone, callback) {
            ...
            $.ajax({
                url: baseURL +
                    "/sys/teacherchannel/queryTeacherRecommendedSecond",
                dataType: "json",
                type: "GET",
                // async: false,
                data: {
...
var vm = new Vue({
    ...
    methods: {
        getRoomIDAndUrl: function (val) {
            this.loadingPageAll = true;
            this.roomUrl = val.roomId;
            this.roomID = val.roomUrl;
            this.loadingPageAll = false;
        },
        interviewedSubmit: function () {
            this.loadingPageAll = true;
            var _this = this;
            $.ajax({
                url: "" + baseURL + "sys/teacherchannel/updateSecondInterview",
                type: "POST",
                dataType: "json",
                headers: {
                    "token": token
        ...
        getOperList: function () {
            ...
            $.ajax({
                ...
                url: "" + baseURL + "/sys/user/queryUserByRoleId",
                success: function (r) {
                    vm.loadingPageAll = false;
        ...
        createInterviewSubmit: function () {
            ...
            $.ajax({
                ...
                url: "" + baseURL + "/sys/teacherchannel/arrangeSecondInterview",
                success: function (r) {
                    console.log(r);
                    if (r.status == 200) {
