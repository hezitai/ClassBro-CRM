  ...
  Calendar.prototype.drawCategories = function() {
    ...
    if (this.conf.categories.enable){
      ...
      for (var i=0; i<this.eventCategoryColor.length; i++){
        ...
        var startTime = "2019-02-01 00:00:00";
        var endTime = "2019-02-28 23:59:59";
        $.ajax({
          url: ""+baseURL+"/sys/teacherchannel/queryTeacherRecommendedFirst",
          dataType: "json",
          type: "get",
          async: false,
          data: {
          ...
          success: function (json) {
            json.body.forEach(function(element){
              if(element.category == this.eventCategoryColor.category){
                label.css('background', element.userColor);
              }
            })
          }
        });
      ...
      success: function (json) {
          ...
          lists.forEach(function(element){
            if(category == element.category){
              li.attr('data-color', element.userColor);
              li.css('background', element.userColor);
            }
          })
      }
    });
