...
jQuery(document).ready(function ($) {
    ...
    Calendar.prototype.monthDrawWeekNumbers = function () {
        ...
        while (parseInt(time.format('MM')) == month) {
            ...
            li.append($('<span>')
                ...
                .text("week" +time.format(this.conf.month.weekline.format)).css('margin-top', this.conf.month.weekline.heightPx / 2 + 'px')
            );
    ...
    Calendar.prototype.weekDrawDaynotes = function () {
        for (var i = 0; i < this.daynotes.length; i++) {
            ...
            if (day === false) {
                ...
                color = this.getDaynoteCategoryColor(i);
                ...
                this.drawEventsOrDaynotes(
                    ...
                    e.content,
                    e.category,
                    this.getDaynoteCategoryColor(i),
                    '',
    ...
    Calendar.prototype.getCategoryColor = function (index, object, colors, color) {
        console.log(colors)
        var count = 0;
        var userColor = "#000000";
        $.each(this.events, function (inx, value) {
            if (index == value.category) {
                count ++;
                userColor = value.userColor;
            }
        });
        if (count == 0) {
            return this.events[index].userColor;
        } else {
            return userColor;
        }
    };
    ...
    Calendar.prototype.determineEventsCategories = function () {
        ...
        for (var i = 0; i < categories.length; i++) {
            ...
            eventCategory.color = this.getEventCategoryColor(categories[i])
            this.eventCategoryColor.push(eventCategory);
        }
    };

    Calendar.prototype.getEventCategoryColor = function (index) {
        return this.getCategoryColor(index, this.eventCategoryColor, this.conf.colors.events, '#000000');
    };
    ...
    Calendar.prototype.getDaynoteCategoryColor = function (index) {
        return this.getCategoryColor(index, this.daynoteCategoryColor, this.conf.colors.daynotes, '#FEFEFE');
    };
